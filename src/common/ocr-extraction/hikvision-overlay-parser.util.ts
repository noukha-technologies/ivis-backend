...
const VEHICLE_TYPE_FIXES: Record<string, string> = {
    '3edan': 'Sedan', sedan: 'Sedan', suv: 'SUV', mpv: 'MPV',
    truck: 'Truck', bus: 'Bus', van: 'Van', pickup: 'Pickup', hatchback: 'Hatchback',
};

export function normalizeOcrOverlayText(text: string): string {
    let flat = text.replace(/\r/g, ' ').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
    flat = flat.replace(/(\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2}:)(\d)\s+(\d)\b/g, '$1$2$3');
...
type LabelHit = { field: keyof HikvisionOverlayFields; start: number; valueStart: number; spec: LabelSpec };
...
function collectLabelHits(flat: string): LabelHit[] {
    ...
    for (const spec of LABEL_SPECS) {
        ...
        while ((match = re.exec(flat)) !== null) {
            hits.push({ field: spec.field, start: match.index, valueStart: match.index + match[0].length, spec, labelLen: match[0].length });
        }
    }
    ...
    for (const hit of hits) {
        ...
        if (deduped.length > 0 && hit.start < deduped[deduped.length - 1].valueStart) continue;
        deduped.push({ field: hit.field, start: hit.start, valueStart: hit.valueStart, spec: hit.spec });
        lastStart = hit.start;
    }
    return deduped;
}

function normalizeVehicleType(raw: string | null | undefined): string | undefined {
    ...
    if (/^[A-Za-z]{2,}$/.test(word)) return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
    return word;
}

function normalizePlateSize(raw: string | null | undefined): string | undefined {
    if (!raw?.trim()) return undefined;
    const value = trimFieldValue(raw).replace(/^e:/i, '');
    const lower = value.toLowerCase();
    if (/\blong\b/.test(lower) || lower === 'l' || lower.startsWith('lon')) return 'Long';
    if (/\bshort\b/.test(lower) || lower.startsWith('sho')) return 'Short';
    return undefined;
}

function normalizePlateType(raw: string | null | undefined): string | undefined {
    if (!raw?.trim()) return undefined;
...
const LABEL_SPECS: LabelSpec[] = [
    { field: 'plateNumber', pattern: /Plate\s*No\.?\s*:?\s*/i, transform: (raw) => raw.replace(/[^A-Za-z0-9؀-ۿ-]/g, '').toUpperCase() },
    { field: 'captureTimeLabel', pattern: /Capture\s*Time\s*:?\s*/i, transform: (raw) => { const m = trimFieldValue(raw).match(/^(\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2}:\d{2})/); return m?.[1]; } },
    { field: 'vehicleColour', pattern: /Vehicle\s*Colou?r\s*:?\s*/i, transform: (raw) => normalizeAnprColour(raw) ?? undefined },
    { field: 'vehicleType', pattern: /Vehicle\s*Type\s*:?\s*/i, transform: (raw) => normalizeVehicleType(raw) },
    { field: 'vehicleBrand', pattern: /(?:Vehicle|Uehicle)\s*Brand\s*:?\s*/i, transform: (raw) => normalizeVehicleBrand(raw) },
    { field: 'direction', pattern: /Moving\s*Direction\s*:?\s*/i, transform: (raw) => { const v = trimFieldValue(raw).split(/\s+/)[0]; return v ? v.charAt(0).toUpperCase() + v.slice(1).toLowerCase() : undefined; } },
    { field: 'confidence', pattern: /Confidence\s*:?\s*/i, transform: (raw) => { const digits = trimFieldValue(raw).match(/(\d{1,3})/); return digits ? parseInt(digits[1], 10) : undefined; } },
    { field: 'plateColour', pattern: /Plate\s*Colou?r\s*:?\s*/i, transform: (raw) => normalizeAnprColour(raw) ?? undefined },
    { field: 'plateSize', pattern: /Plate\s*(?:Size|Siz|3ize)\s*:?\s*/i, transform: (raw) => normalizePlateSize(raw) },
    { field: 'plateType', pattern: /Plate\s*Type\s*:?\s*/i, transform: (raw) => normalizePlateType(raw) },
    { field: 'province', pattern: /Province\s*:?\s*/i, transform: (raw) => trimFieldValue(raw) },
    { field: 'category', pattern: /Category\s*:?\s*/i, transform: (raw) => trimFieldValue(raw).replace(/[^A-Za-z0-9]/g, '').toUpperCase() },
];
...
export function parseHikvisionOverlayFields(text: string): HikvisionOverlayFields {
    ...
    for (let i = 0; i < hits.length; i++) {
        ...
        if (!raw) continue;
        const transformed = spec.transform ? spec.transform(raw) : trimFieldValue(raw);
...
export function parseCaptureTimeLabel(label: string): Date | null {
    ...
    const dt = new Date(parseInt(yyyy, 10), parseInt(mm, 10) - 1, parseInt(dd, 10), parseInt(hh, 10), parseInt(mi, 10), parseInt(ss, 10));
