...
export type JpegOcrExtractionDebug = {
    ...
    selection?: { chosenPlate?: string; source?: string; candidates: OcrCandidateDebug[] };
    notes?: string[];
};
...
export function formatOcrExtractionDebug(report: JpegOcrExtractionDebug): string {
    const lines: string[] = [
    ...
    if (report.detectionOverlay) {
        lines.push(
            formatSection('Detection image — overlay strip OCR (raw)', [report.detectionOverlay.rawOcrText || '(empty)']),
            formatSection('Detection image — parsed overlay fields', [...Object.entries(report.detectionOverlay.parsedFields).map(([k, v]) => `${k}: ${v ?? '—'}`)]),
        );
    }
    if (report.plateCrop) {
        lines.push(formatSection('Plate crop — digit region OCR (raw)', [report.plateCrop.rawDigitOcr || '(empty)']));
        if (report.plateCrop.rawFullOcr !== undefined) {
            lines.push(formatSection('Plate crop — full image OCR (raw)', [report.plateCrop.rawFullOcr || '(empty)']));
        }
        lines.push(formatSection('Plate crop — normalized', [`method: ${report.plateCrop.method}`, `plate: ${report.plateCrop.normalizedPlate ?? '—'}`]));
    }
    if (report.scene) {
        lines.push(
            formatSection('Scene image — OCR (raw)', [report.scene.rawOcrText || '(empty)']),
            formatSection('Scene image — normalized plate', [report.scene.normalizedPlate ?? '—']),
        );
    }
    if (report.selection) {
        lines.push(formatSection('Final plate selection', [
            `chosen: ${report.selection.chosenPlate ?? '—'}`,
            `source: ${report.selection.source ?? '—'}`,
            ...report.selection.candidates.map((c) => `- ${c.source}: raw="${c.raw ?? ''}" normalized="${c.normalized ?? ''}"`),
        ]));
    }
