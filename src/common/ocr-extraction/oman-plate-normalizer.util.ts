...
function isLongPlateContext(hints: PlateNormalizeHints): boolean {
  ...
  if (size.includes('long') || size === 'l' || size.startsWith('lon'))
    return true;
...
function isCategoryBleedOfPlateSuffix(
  plateSuffix: string,
  category: string,
): boolean {
  return (
...
function extractLongFormatDigits(
  plate: string,
  category?: string | null,
): string | null {
  let working = plate;
  ...
  for (let i = 0; i < working.length; i++) {
    ...
    if (/\d/.test(ch)) {
      if (cat && next === 'R' && cat.startsWith('R') && digits.length >= 3)
        break;
      digits += ch;
      continue;
    }
...
function normalizeStandardPlate(plate: string): string {
  ...
  if (prefixMatch)
    return `${prefixMatch[1]}${normalizeDigitsSegment(prefixMatch[2])}`;
  const suffixMatch = plate.match(/^(\d+)([A-Z]{1,3})$/);
  if (suffixMatch)
    return `${normalizeDigitsSegment(suffixMatch[1])}${suffixMatch[2]}`;
...
function shouldComposeFromCategory(
  ...
  if (hints.appendCategory === false || !isKnownCategory(category))
    return false;
  ...
  if (suffixMatch) {
    ...
    if (suffix.length >= 2 && suffix !== category && !isLongPlateContext(hints))
      return true;
  ...
  if (
    category &&
    plate.endsWith(category) &&
    plate.length > category.length + 2
  )
    return true;
  if (digits && plate !== `${digits}${category}`) {
    const withoutCat =
      category && plate.endsWith(category)
        ? plate.slice(0, -category.length)
        : plate;
    if (
      withoutCat !== digits &&
      extractLongFormatDigits(plate, category) === digits
    )
      return true;
  }
  return false;
...
export function normalizeOcrPlateNumber(
  ...
  if (
    ...
    digits &&
    digits.length >= 3 &&
    shouldComposeFromCategory(plate, category, hints, digits)
  ) {
...
export function pickBestPlateCandidate(
  ...
  for (const c of candidates) {
    ...
    const hints: PlateNormalizeHints = {
      ...(c.hints ?? {}),
      appendCategory:
        c.source === 'plate_crop' ? false : c.hints?.appendCategory,
    };
    ...
    if (c.hints && isLongPlateContext(c.hints) && c.source === 'overlay')
      score += 15;
  ...
  if (overlay && crop) {
    ...
    else if (oDigits !== cDigits) {
      crop.score -= 40;
      overlay.score += 25;
    } else if (
      catEndsWith(overlay.plate, overlay.hints?.category) &&
      !catEndsWith(crop.plate, crop.hints?.category)
    )
      overlay.score += 20;
    else crop.score += 15;
  }
  ...
  return scored[0]
    ? { plate: scored[0].plate, source: scored[0].source }
    : null;
}
