...
export class ImageProcessorService {
  async compressAnprImages(
    files: Record<string, Buffer>,
  ): Promise<ProcessedAnprImagesDto> {
    const plateInput =
      files['licensePlatePicture.jpg'] ?? files['licensePlatePicture'];
    const sceneInput =
      files['detectionPicture.jpg'] ?? files['detectionPicture'];
  ...
  async saveCompressedImages(
    ...
    const safePlate =
      (plateNumber || 'UNKNOWN').replace(/[^A-Za-z0-9]/g, '') || 'UNKNOWN';
