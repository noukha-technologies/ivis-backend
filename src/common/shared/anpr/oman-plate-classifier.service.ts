...
export class OmanPlateClassifierService {
  ...
  classify(
    plateNumber: string,
    plateColorRaw?: string,
  ): OmanPlateClassification {
    ...
    if (
      colorResult.ev &&
      !typeResult.category.startsWith('GOV') &&
      !typeResult.alert
    ) {
      finalPlateType =
        colorResult.colorEntry?.note.split('—')[0].trim() ??
        typeResult.typeName;
