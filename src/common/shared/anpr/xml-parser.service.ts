...
export class XmlParserService {
  ...
  parseAnprXml(xmlBuffer: Buffer): AnprNormalizedEventDto {
    const payload = this.parser.parse(
      xmlBuffer.toString(),
    ) as HikvisionAlertPayload;
    const alert = payload.EventNotificationAlert;

    if (!alert) {
      throw new BadRequestException(
        'Invalid XML: EventNotificationAlert missing',
      );
    }
    ...
    if (eventType !== 'ANPR') {
      throw new BadRequestException(
        `Unsupported eventType: ${eventType || 'empty'}`,
      );
    }
    ...
    if (Number.isNaN(captureTime.getTime())) {
      throw new BadRequestException(
        'Invalid XML: dateTime is not a valid ISO date',
      );
    }
    ...
    if (confidence === undefined) {
      throw new BadRequestException(
        'Invalid XML: ANPR.confidenceLevel missing',
      );
    }
  ...
  parseDeviceInfo(xml: string | Buffer): {
    model?: string;
    serialNumber?: string;
    firmware?: string;
  } {
