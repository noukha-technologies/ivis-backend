...
export function stripCentreFromFtpRoot(
  ...
  if (
    root.endsWith(winSuffix) ||
    root.toLowerCase().endsWith(winSuffix.toLowerCase())
  ) {
    return root.slice(0, root.length - centre.length).replace(/[/\\]+$/, '');
  }
  return root;
...
export function stripCameraPathFromFtpRoot(
  ...
  const root = stripCentreFromFtpRoot(ftpDirectory, centreCode);
...
export function resolveCameraFolder(
  ...
  const candidates = resolveCameraFolderCandidates(
    ftpRoot,
    centreCode,
    ipAddress,
  );
...
export function shouldUseMountMode(camera: {
  ftpDirectory?: string | null;
}): boolean {
