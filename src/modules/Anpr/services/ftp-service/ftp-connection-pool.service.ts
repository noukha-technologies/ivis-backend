...
export class FtpConnectionPoolService implements OnApplicationShutdown {
    ...
    async getConnection(camera: CameraEntity): Promise<Client> {
        ...
        return new Promise((resolve, reject) => {
            ...
            client.once('error', (err: Error) => {
                this.logger.error(
                    `FTP error for ${camera.cameraCode}: ${err.message}`,
                );
    ...
    async downloadFile(camera: CameraEntity, remoteFile: string): Promise<Buffer> {
        const client = await this.getConnection(camera);
        return new Promise((resolve, reject) => {
    ...
    async ensureDirectory(camera: CameraEntity, remotePath: string): Promise<void> {
        const client = await this.getConnection(camera);
        return new Promise((resolve, reject) => {
