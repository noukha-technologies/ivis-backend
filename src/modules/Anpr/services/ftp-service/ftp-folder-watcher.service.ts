...
export class FtpFolderWatcherService implements OnApplicationShutdown {
    ...
    async startAll(cameras?: CameraEntity[]): Promise<void> {
        const list = cameras ?? (await this.methodConfig.findActiveCamerasWithFtp());
        if (list.length === 0) {
            return;
        }
    ...
    private logCameraPathStatus(camera: CameraEntity): void {
        ...
        const cameraFolderOk = cameraFolder ? this.pathReadable(cameraFolder) : false;
    ...
    private startMountWatch(camera: CameraEntity, handle: WatcherHandle): void {
        ...
        if (!dir) {
            this.logger.error(
                `[FTP Watcher] No watch path for ${camera.cameraCode}`,
            );
            return;
        }
        ...
        if (!this.pathReadable(dir)) {
            ...
            if (!handle.interval) {
                handle.interval = setInterval(() => {
                    ...
                    if (
                        next &&
                        this.pathReadable(next) &&
                        next !== handle.watchedPath
                    ) {
                        if (handle.interval) {
                            clearInterval(handle.interval);
