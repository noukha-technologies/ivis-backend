...
export class AnprService {
  ...
  async handleHikvisionWebhookPush(req: Request, res: Response): Promise<void> {
    const cameraCodeHint =
      await this.anprWebhookService.parseHikvisionWebhookCameraCode(req);
    ...
    void (async () => {
      ...
      try {
        ...
        this.logger.error(
          chalk.red(
            `[ANPR Controller] Failed to read webhook body: ${message}`,
          ),
        );
        if (!res.headersSent) {
          res.status(400).end();
      ...
      await this.anprWebhookService.processIncomingEvent(
        req,
        rawBody,
        cameraCodeHint,
      );
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(
        chalk.red(`[ANPR Controller] Async processing failed: ${message}`),
      );
      if (!res.headersSent) {
        res.status(200).end();
      }
