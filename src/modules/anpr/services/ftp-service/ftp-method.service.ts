...
export class FtpMethodService {
    ...
    async buildEventFromRemoteXml(
        ...
        const xmlDir = this.remoteDirname(remoteXmlPath, camera.ftpDirectory ?? '/');
    ...
    async buildEventFromJpegBundle(
        ...
        if (debugAnchor) {
                ...
                selection: {
                    ...
                    candidates: [
                        ...
                        },
                        {
                            ...
                            normalized: bestPlate?.source === 'overlay'
                                ? bestPlate.plate
            ...
            if (txtPath) {
                this.logger.log(
                    `[FTP JPEG] OCR debug written: ${txtPath}`,
                );
            }
        }
        ...
        const scenePath =
            bundle.vehiclePicturePath ?? bundle.vehicleDetectionPath;
        if (scenePath) {
