...
import {
  AnprRawMultipartInterface,
  ParsedAnprEvent,
  WebhookRawCapture,
} from '../../../../common/interfaces/anpr.interface';
...
export class AnprWebhookService {
  ...
  private assertHttpPushCamera(camera: CameraEntity, context: string): void {
    if (
      !this.methodConfig.acceptsInbound(camera, CameraIntegrationMethod.PUSH)
    ) {
  ...
  private logWebhookResolution(
    identifier: string,
    camera: CameraEntity,
    reason: WebhookResolveReason,
  ): void {
    if (identifier === camera.cameraCode && reason === 'camera_code') {
      this.logger.log(
        `[ANPR Service] Webhook resolved ${camera.cameraCode} (${reason})`,
      );
      return;
    }
    this.logger.log(
      `[ANPR Service] Webhook resolved identifier "${identifier}" → camera ${camera.cameraCode} (centre ${camera.centreCode}, ${reason})`,
    );
  }
  ...
  private disambiguateCamerasByRequestIp(
    ...
    if (ips.length === 0) {
      throw new BadRequestException(
        `Multiple cameras match ${context}; could not disambiguate without request IP`,
      );
    }
    ...
    const matched = cameras.filter((c) => {
      ...
      return ips.some(
        (ip) => ip === camIp || ip.replace(/^::ffff:/i, '') === camIp,
      );
    });
    ...
    throw new BadRequestException(
      `Multiple cameras match ${context} (${codes}); disambiguate with a unique cameraCode or IP`,
    );
  }
   ...
   */
  private passesConfidenceFilter(
    eventDto: ParsedAnprEvent,
    threshold = 80,
  ): boolean {
   ...
   */
  private dumpHikvisionWebhookPayload(
    req: Request,
    cameraCode: string,
    payload: AnprRawMultipartInterface,
  ): void {
    ...
    const folder = path.join(
      dir,
      `${stamp}_${cameraCode.replace(/[^\w-]/g, '_')}`,
    );
    ...
    const meta = {
      ...
      receivedAt:
        payload.meta?.receivedAt?.toISOString() ?? new Date().toISOString(),
    };
    fs.writeFileSync(
  ...
  private async resolveCameraForWebhook(
    ...
    if (trimmed) {
      ...
      if (aliasTarget) {
        ...
        if (byAlias) {
          ...
          this.logWebhookResolution(
            trimmed,
            byAlias,
            WebhookResolveReason.ALIAS,
          );
          return byAlias;
        }
      }
      ...
      if (byCameraCode) {
        this.assertHttpPushCamera(
          byCameraCode,
          'HTTP push webhook (camera code)',
        );
        this.logWebhookResolution(
          trimmed,
          byCameraCode,
          WebhookResolveReason.CAMERA_CODE,
        );
        return byCameraCode;
      }

      const byCentre = await this.cameraRepo
        .find({
          where: {
            status: 'Active',
            is_deleted: false,
      ...
      if (fromCentre) {
        this.logWebhookResolution(
          trimmed,
          fromCentre,
          WebhookResolveReason.CENTRE_CODE,
        );
        return fromCentre;
      }
    ...
    this.logWebhookResolution(
      fromIp.ipAddress,
      fromIp,
      WebhookResolveReason.REQUEST_IP,
    );
    return fromIp;
  }
   ...
   */
  public async parseHikvisionWebhookCameraCode(
    req: Request,
  ): Promise<string | null> {
  ...
  public async saveRawWebhookCapture(
    req: Request,
    body: Buffer,
    cameraCodeHint?: string | null,
  ): Promise<WebhookRawCapture> {
    ...
    const meta = {
      ...
      bodyPreviewHex: body
        .subarray(0, Math.min(256, body.length))
        .toString('hex'),
  ...
  async processIncomingEvent(
    ...
    try {
      const fromQuery =
        cameraCodeHint ??
        (await this.parseHikvisionWebhookCameraCode(req)) ??
        '';
      ...
      this.logger.error(
        chalk.red(
          `[ANPR Service] Failed to process incoming event: ${message}`,
        ),
      );
      throw error;
    }
  ...
  async processPost(
    ...
    try {
      this.logger.debug(
        `[HTTP Push] Starting multipart parse from ${req.ip} (cameraCode: ${cameraCode})`,
      );
      ...
      this.logger.debug(
        `[HTTP Push] Multipart parsed: XML ${payload.xmlBuffer.length}B, ${Object.keys(payload.files).length} files.`,
      );
  ...
  async processEvent(eventDto: ParsedAnprEvent): Promise<boolean> {
    try {
      ...
      if (!camera) {
        ...
        if (required && !this.methodConfig.acceptsInbound(camera, required)) {
          this.methodConfig.logInboundRejection(
            camera,
            required,
      ...
      const ocr = (
        eventDto.rawPayload as { ocrParsed?: Record<string, unknown> } | null
      )?.ocrParsed;
        ...
        capture = await this.anprCaptureService.create(
          {
            ...
            plate_type:
              eventDto.plateType ?? (ocr?.plateType as string) ?? undefined,
  ...
  private broadcastCapture(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(
        `[ANPR Service] WebSocket broadcast failed: ${message}`,
      );
