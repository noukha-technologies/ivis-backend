...
export class CentralOnboardingHttpClientService {
  ...
  async verifyCentral(
    email: string,
    password: string,
  ): Promise<VerifyCentralResult> {
    return this.post<VerifyCentralResult>('/onboarding/verify-central', {
      email,
      password,
    });
  }
  ...
  async resolveReScopedRow(
    ...
    const { row } = await this.post<{ row: Record<string, unknown> }>(
      '/onboarding/resolve-rescoped-row',
      {
        email,
        centreId,
      },
  ...
  async confirm(
    ...
    return this.post<OnboardingConfirmResult>('/onboarding/confirm', {
      email,
      password,
    });
  }
  ...
  ): Promise<{ pullSessionId: string }> {
    return this.post('/onboarding/pull/start', {
      pullToken,
  ...
  async pullChunk(
    ...
    return this.post('/onboarding/pull/chunk', {
      pullSessionId,
      entityKey,
      cursor,
    });
  ...
  async pullByIds(
    ...
    const { rows } = await this.post<{ rows: Record<string, unknown>[] }>(
      '/onboarding/pull/by-ids',
      {
        pullSessionId,
        entityKey,
        ids,
  ...
  async pullComplete(
    pullSessionId: string,
  ): Promise<OnboardingPullCompleteResult> {
  ...
  private async post<T>(
    ...
    this.logger.log(
      `--> POST ${url} ${JSON.stringify(safeBody)}`,
      CentralOnboardingHttpClientService.context,
    );
    ...
    this.logger.log(
      `<-- POST ${url} 200 ${JSON.stringify(envelope)}`,
      CentralOnboardingHttpClientService.context,
    );
